...
export async function GET(request: NextRequest) {
    ...
    try {
      ...
      const collection = db.collection("Pending_Prospects"); // Change to your actual collection name
  
      const prospect = await collection.findOne({ entity_id });
  
      if (!prospect) {
        return NextResponse.json({ response: "Pending Prospect not found" }, { status: 200 });
      }
